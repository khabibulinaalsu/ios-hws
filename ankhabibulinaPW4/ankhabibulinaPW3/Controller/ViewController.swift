...
final class WelcomeViewController: UIViewController {
    ...
    private func setupMenuButtons() {
        ...
        notesButton.addTarget(self, action: #selector(notesViewButtonPressed), for: .touchUpInside)
    ...
    private func setupColorControlSV() {
        ...
        NSLayoutConstraint.activate(
            [colorPaletteView.topAnchor.constraint(equalTo: incrementButton.bottomAnchor, constant: 24),
             ...
             colorPaletteView.bottomAnchor.constraint(equalTo: colorsButton.topAnchor, constant: -24)
         ])
    ...
    private func changeColor(_ slider: ColorPaletteView) {
        UIView.animate(
            ...
            animations: {
                ...
                self.noteViewController.view.backgroundColor = slider.chosenColor
            }
        )
    }
    
    @objc
    private func notesViewButtonPressed(_ sender: UIButton) {
        
        present(noteViewController, animated: true)
        //noteViewController.labelView.backgroundColor = self.view.backgroundColor
        let generator = UIImpactFeedbackGenerator(style: .medium)
        generator.impactOccurred(intensity: 1)
        if colorPaletteView.isHidden == false {
            paletteButtonPressed(colorsButton)
        }
    }
